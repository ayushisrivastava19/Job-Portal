...
function handleHttpHeader(lowerCasedKey, value, sendPii, isCookieSubKey = false) {
  const snippetsForSensitivity = isCookieSubKey ? [...filteringSnippets.SENSITIVE_KEY_SNIPPETS, ...filteringSnippets.SENSITIVE_COOKIE_NAME_SNIPPETS] : filteringSnippets.SENSITIVE_KEY_SNIPPETS;
  const isSensitive = sendPii ? snippetsForSensitivity.some((snippet) => lowerCasedKey.includes(snippet)) : [...filteringSnippets.PII_HEADER_SNIPPETS, ...snippetsForSensitivity].some((snippet) => lowerCasedKey.includes(snippet));
  if (isSensitive) {
    return "[Filtered]";
